...
def plot_history(steps_list, reward_list, success_list, epsilon_list, target_win_ratio, min_steps_num, save_path):
    plt.figure(figsize=[9,16])
    plt.subplot(411)
    plt.plot(pd.Series(steps_list))
    ...
    plt.subplot(412)
    plt.plot(pd.Series(reward_list))
    ...
    plt.subplot(413)
    plt.plot(pd.Series(success_list))
    plt.title('Wins per training episode')
    plt.ylabel('If won')
    plt.axhline(y=target_win_ratio, color='r', linestyle='-', label=f'Early stop condition: {target_win_ratio*100:.2f}%')
    plt.legend()
    plt.xlabel('Episode')
